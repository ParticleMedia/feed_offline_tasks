...
def dump_idf(path, cate_idf):
    with open(path, 'w') as outfd:
        for cate, idf in cate_idf.items():
            outfd.write('%s\t%f\n' % (cate, idf))
    return

def main(total_user_cnt, category_user_cnt_file, category_idf_file):
    ...
    for line in sys.stdin:
        ...
        if len(cate_list) > MAX_CATE_CNT:
            cate_list = cate_list[0:MAX_CATE_CNT]
        try:
    ...
    if category_idf_file is not None:
        dump_idf(category_idf_file, cate_idf_dict)
...
if __name__ == "__main__":
    if len(sys.argv) < 4:
        sys.stderr.write("Usage python %s total_user_cnt category_user_cnt_file category_idf_file\n" % sys.argv[0])
        sys.exit(1)
    # Execute Main functionality
    total_user_cnt = int(sys.argv[1])
    category_user_cnt_file = sys.argv[2]
    category_idf_file = sys.argv[3]
    if total_user_cnt <= 0:
        sys.stderr.write("total_user_cnt: %d should be positive" % total_user_cnt)
        sys.exit(1)

    main(total_user_cnt, category_user_cnt_file, category_idf_file)
