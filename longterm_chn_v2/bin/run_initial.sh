...
function do_tfidf() {
    ...
    local cate_idf_file=${output_dir}/${idf_file}
    cat ${tf_local_dir}/part-* | python ${LOCAL_BIN_PATH}/tfidf_brief.py ${total_key_cnt} ${cate_key_cnt_file} ${cate_idf_file} 1>${output_dir}/${out_filename}
    cat ${output_dir}/${out_filename} | python ${LOCAL_BIN_PATH}/tfidf_percentile.py 20 1>${output_dir}/${out_filename}.percentile

    # upload to hdfs
    local hdfs_dest_dir=${HDFS_WORK_PATH}/${key}_tfidf_${calc_days}/${DATE_FLAG}
    ${HDFS_BIN} dfs -mkdir -p ${hdfs_dest_dir}
...
function write_to_ups() {
    ...
    cat ${data_file} | ${LOCAL_BIN_PATH}/write_profile --host=${UPS_HOST} --port=${UPS_PORT} --profile=${UPS_PROFILE} --version=${UPS_VERSION} --batch=${UPS_BATCH} --format=${UPS_FORMAT} --worker=${UPS_WORKER}
    ret=$?
    if [ ${ret} -ne  0 ]; then
        return ${ret}
    fi

    local percentile_file=${data_file}.percentile
    if [  -f ${percentile_file} ]; then
        local nfs_dir=/mnt/models/monica/dict/percentile
        mkdir -p ${nfs_dir}
        cp ${percentile_file} ${nfs_dir}/${UPS_PROFILE}.percentile
    fi
    return ${ret}
}
...
function process() {
    local module_conf=$1
    local ret=0
    local initial_days=3
    local process_cjv_conf=${LOCAL_CONF_PATH}/process_cjv_initial.conf

    # write your own logic here
    for ((i=1;i<=${initial_days};i++)); do
      export PROCESSING_DATE=`date +%Y%m%d -d "-$i days"`
      run_mapred ${module_conf} ${process_cjv_conf}
      ret=$?
      if [ ${ret} -ne  0 ]; then
          echo "process ${PROCESSING_DATE} cjv failed. ret[${ret}]" 1>&2
          return ${ret}
      fi
      echo "process ${PROCESSING_DATE} cjv succeed." 1>&2
    done

    # # key calc_days click_threshold out_filename
    do_tfidf ${module_conf} uid 100 3 user_3m.txt
    ret=$?
    if [ ${ret} -ne  0 ]; then
        return ${ret}
    fi
