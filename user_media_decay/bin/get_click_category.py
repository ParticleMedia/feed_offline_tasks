...
#STAT_FIELDS = ['domain','sc_channels','channels','nlu_tags','nlu_tags_scores','text_category']
STAT_FIELDS = ['media_id','text_category']

def getDocFeatureWithRetry(docid, retryCnt):
    for i in range(0, retryCnt):
        docInfo = getDocFeature(docid, 'is_local_news,%s' % ','.join(STAT_FIELDS))
        if docInfo is not None:
            ...
            #ignore doc which has no media
            if "media_id" not in docInfo or docInfo["media_id"] == "":
                return None
            return docInfo
    return None
...
class DocInfo:
    ...
    def isLocal(self):
        if self.docDict is None:
            ...
            return self.docDict.get('is_local_news', '') == 'true' and not isSport(self.docDict)
