...
function wait_foryou_data(){
    fy_path=${HDFS_ROOT_PATH}/user_tw_chn/time_decay_history/${DATE_FLAG}/_SUCCESS
...
function cal_cooccur() {
    ...
    cp ${LOCAL_DATA_PATH}/cooccur/${DATE_FLAG}/part-00000 /mnt/models/foryou/chn_2_chn.txt
    return $?
}
...
function cleanup() {
    if [ -n "${LOG_CLEANUP_DAY}" ]; then
        ...
        if [ -n "${LOCAL_DATA_PATH}" ]; then
            rm -r ${LOCAL_DATA_PATH}/cooccur/${LOG_CLEANUP_DAY}
            rm -r ${LOCAL_DATA_PATH}/idf/${LOG_CLEANUP_DAY}
        fi
