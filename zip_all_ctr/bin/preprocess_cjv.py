...
def filter_cjv(item):
    ...
    #    condition = item['push_view'].get('condition', '')
    if not condition.startswith('local'):
        return False
    return True

def isClick(cjvData):
    if cjvData.get('clicked', False) and cjvData.get('pv_time', 0) > 2000:
        return True
...
def process_cjv(cjvData):
    ...
    key = cjvData['nr_view'].get('key', '').strip('"').split('@')[0]
    if len(key) == 0:
        hadoop_counter.increase_counter('cjv', 'filter')
        return
    ...
    outArr = [docid, key, '%d' % ts,'%d' % clicked]
    print '\t'.join(outArr)
...
if __name__ == "__main__":
    for line in sys.stdin:
        ...
        if line.find('"checked":true,') < 0:
            hadoop_counter.increase_counter('cjv', 'no_check')
            continue
