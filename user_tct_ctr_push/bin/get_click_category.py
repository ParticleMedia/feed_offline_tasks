...
def getDocFeatureWithRetry(docid, retryCnt):
    for i in range(0, retryCnt):
        docInfo = getDocFeature(docid, KEY)
        if docInfo is not None:
            if KEY in docInfo:
                docInfo[KEY] = json.dumps(docInfo[KEY])
            return docInfo
    return None
...
if __name__ == "__main__":
    ...
    for line in sys.stdin:
        ...
        if curDocInfo is None:
            ...
            continue
        try:
            if KEY in curDocInfo:
                # swap splits[0] and splits[1]
                uid = splits[1]
                splits[0] = uid
                splits[1] = docid
                splits.append(curDocInfo[KEY])
