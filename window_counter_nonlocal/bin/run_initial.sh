...
function dump_cjv_from_hive() {
    local cjv_date=`date -d ${PROCESSING_DATE} +%Y-%m-%d`
    local hdfs_cjv_path=${HDFS_WORK_PATH}/cjv/${PROCESSING_DATE}
    local hive_sql="SELECT cjv.doc_id, cjv.user_id, unix_timestamp(cjv.ts), cjv.pv_time, cjv.cv_time, cjv.clicked, cjv.liked, cjv.shared, cjv.thumbed_up, cjv.thumbed_down FROM warehouse.online_cjv_hourly as cjv WHERE cjv.joined = 1 and cjv.checked = 1 and cjv.ctype = 'news' and (cjv.channel_name = 'foryou' or (cjv.channel_name = 'channelnew' and cjv.channel_id in ('k122515', 'k122516', 'k122519', 'k122521', 'k122522', 'k122524', 'k122525', 'k122551', 'k122552'))) and cjv.nr_condition not LIKE 'local%' and cjv.nr_condition != 'statechannel' and cjv.user_id > 0 and cjv.pdate = '${cjv_date}'"
...
function process() {
    ...
    for ((i=1;i<=${initial_days};i++)); do
        ...
        ret=$?
        if [ ${ret} -ne  0 ]; then
            echo "dump ${PROCESSING_DATE} cjv from hive failed. ret[${ret}]" 1>&2
            return ${ret}
        fi
        echo "dump ${PROCESSING_DATE} cjv from hive succeed." 1>&2
