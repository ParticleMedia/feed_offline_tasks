...
function do_ctr() {
    ...
    if [ "x${WRITE_TO_UPS}" == "xTRUE" ]; then
        ...
        write_to_ups ${data_dir} ${ups_version}
        echo "write_to_ups" >&2
        ret=$?
    fi
}
...
function process() {
    ...
    # module_conf, select_days, check_threshold, click_threshold, user_profile_version
    #do_ctr ${module_conf} 90 20 4 0
    do_ctr ${module_conf} 91 10 0 1
    ret=$?
    if [ ${ret} -ne  0 ]; then
        return ${ret}
    fi
