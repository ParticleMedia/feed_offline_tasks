...
def main(min_ts, field, score_field, filter_logic):
    ...
    for line in sys.stdin:
        ...
        try:
            ...
            domain = docInfo.get(field, '')
            if len(domain) == 0:
                hadoop_counter.increase_counter('map', 'no_domain')
                continue
            if domain.startswith('newsbreak'):
                hadoop_counter.increase_counter('map', 'domain_newsbreak')
                continue
            ...
            outSplits = [splits[0], splits[1], splits[2], json.dumps([domain])]
