...
def main():
    ...
    for line in sys.stdin:
        ...
        try:
            ...
            if 'channels_v2' not in docInfo or len(docInfo.get('channels_v2', [])) == 0:
                hadoop_counter.increase_counter('map', 'no_category')
                continue

            if 'sc_channels_v2' not in docInfo or len(docInfo.get('sc_channels_v2', [])) == 0:
                hadoop_counter.increase_counter('map', 'no_category_sc')
                continue

            if len(docInfo.get('channels_v2', [])) != len(docInfo.get('sc_channels_v2', [])):
            ...
            outSplits = [splits[0], splits[1], splits[2],
                         json.dumps(tuple(zip(docInfo['channels_v2'],normalize_sc_chn(docInfo['sc_channels_v2']))))]
