...
function dump_cjv_from_hive() {
    ...
    local start_date=`date -d "${DATE_FLAG} -90 days" +%Y-%m-%d`
    local hive_sql="SELECT cjv.doc_id, cjv.user_id, unix_timestamp(cjv.ts), cjv.pv_time, cjv.cv_time, cjv.clicked, cjv.liked, cjv.shared, cjv.thumbed_up, cjv.thumbed_down FROM warehouse.online_cjv_hourly as cjv WHERE cjv.joined = 1 and cjv.checked = 1 and cjv.ctype = 'news' and cjv.channel_name = 'foryou'  and cjv.nr_condition not LIKE 'local%' and cjv.nr_condition != 'statechannel' and cjv.user_id > 0 and cjv.pdate >= '${start_date}' and cjv.pdate <= '${CJV_DATE_FLAG}'"
...
function initialize_merge() {
    ...
    local process_cjv_init_conf=${LOCAL_CONF_PATH}/process_cjv_init.conf
    run_mapred ${module_conf} ${process_cjv_init_conf}
    ret=$?
    if [ ${ret} -ne  0 ]; then
        return ${ret}
    fi
